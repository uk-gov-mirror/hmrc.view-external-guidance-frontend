...
import models.ui.{DateInputPage, StandardPage, InputPage, QuestionPage, FormData, SubmittedAnswer}
...
class GuidanceController @Inject() (
  ...
  private def showPage(path: String, pageContext: PageContext)(implicit request:Request[AnyContent]): Future[Result] = {
    ...
    pageContext.page match {
      ...
      case page: DateInputPage => getInputUIComponent(pageContext) match {
        case Some(uiComponent) => {
          Future.successful(
            Ok(dateInputView(page, pageContext, questionName(path), populateForm(uiComponent, questionName(path), pageContext.answer))))
        }
        case _ =>
          logger.error(s"Unable to determine input UI component for process ${pageContext.processCode} on page load")
          Future.successful(InternalServerError(errorHandler.internalServerErrorTemplate))
  ...
  private def createInputView(pec: PageEvaluationContext, inputName: String, formData: Option[FormData], form: Form[_])
    ...
    pageContext.page match {
      ...
      case page: DateInputPage => dateInputView(page, pageContext, inputName, form)
