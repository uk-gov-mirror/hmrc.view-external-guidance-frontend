...
class PassPhraseProtection() {
  ...
  def stanzas(initialLabelIndex: Int):Seq[(String, Stanza)] = Seq(
    ...
    (ChoiceId, ChoiceStanza(
                Seq(Process.StartStanzaId, PageId),
                Seq(ChoiceStanzaTest(s"[label:${Process.PassPhraseLabelName}]", Equals, s"[label:${Process.PassPhraseResponseLabelName}")), false))
  )
  def protect(process: Process): Process = {
    val nxtLabelIndex: Int = process.phrases.length
    process.copy(flow = flow ++ stanzas(nxtLabelIndex), phrases = phrases ++ Vector(Phrase("Enter passphrase", "Rhowch gyfrinair")))
  }
}
