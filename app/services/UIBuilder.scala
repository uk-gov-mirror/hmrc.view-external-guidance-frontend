...
class UIBuilder {
  ...
  private def fromStanzas(stanzas: Seq[VisualStanza], acc: Seq[UIComponent], errStrategy: ErrorStrategy)
    ...
    stanzas match {
      ...
      case (wt: WarningText) :: xs => fromStanzas(xs, acc ++ Seq(fromWarningText(wt)), errStrategy)
  ...
  private def fromWarningText(wt: WarningText)(implicit stanzaIdToUrlMap: Map[String, String], lang: Lang): UIComponent =
    WarningComponent(wt.group.map(wc => TextBuilder.fromPhrase(wc.text)))
