...
class GuidanceControllerSpec extends BaseSpec with ViewFns with GuiceOneAppPerSuite {
  ...
  "Returning to a previously answered Question page in a process" should {
    ...
    "Show the original answer selected" in new QuestionTest {
      MockGuidanceService
        .getPageContext(processId, path, previousPageByLink = false, processId)
        .returns(Future.successful(Right(PageContext(expectedPage,  None, processId, Some("/"), Text(Nil, Nil), processId, processCode, LabelCache(), None, Some(ansIndexZero)))))
  ...
  "Submitting an answered Question page form" should {
    ...
    "return a BAD_REQUEST when submitting page and guidance determines invalid data" in new QuestionTest {
      ...
      MockGuidanceService
        .getPageContext(pec, NoError)
        .returns(PageContext(expectedPage, None, sessionId, Some("/hello"), Text(Nil, Nil), processId, processCode, initialLabels, di))
    ...
    "return a BAD_REQUEST response if trying to submit a page which is not a question" in new QuestionTest {
      ...
      MockGuidanceService
        ...
        .returns(PageContext(standardPage, None, sessionId, Some("/hello"), Text(Nil, Nil), processId, processCode, initialLabels, di))
  ...
  "Calling a valid URL path to an Input page in a process" should {
    ...
    "return an OK response" in new InputTest {
      ...
      MockGuidanceService
        .getPageContext(processId, path, previousPageByLink = false, processId)
        .returns(Future.successful(Right(PageContext(expectedPage, None, sessionId, Some("/"), Text(Nil, Nil), processId, processCode, LabelCache(), di))))
  ...
  "Returning to an input page in a process" should {
    ...
    "Show the original value entered" in new InputTest {
      MockGuidanceService
        .getPageContext(processId, path, previousPageByLink = false, processId)
        .returns(Future.successful(Right(PageContext(expectedPage, None, sessionId, Some("/"), Text(Nil, Nil), processId, processCode, initialLabels, di, Some(enteredValue)))))
  ...
  "Date Input processing" should {
    ...
    "Calling a valid URL path to an Date Input page in a process" should {
      ...
      "return an OK response" in new DateInputTest {
        ...
        MockGuidanceService
          .getPageContext(processId, path, previousPageByLink = false, processId)
          .returns(Future.successful(Right(PageContext(expectedPage, None, sessionId, Some("/"), Text(Nil, Nil), processId, processCode, initialLabels, di))))
    ...
    "Returning to a date input page in a process" should {
      ...
      "Show the original value entered" in new DateInputTest {
        MockGuidanceService
          .getPageContext(processId, path, previousPageByLink = false, processId)
          .returns(Future.successful(Right(PageContext(expectedPage, None, sessionId, Some("/"), Text(Nil, Nil), processId, processCode, initialLabels, di, Some(enteredDate)))))
