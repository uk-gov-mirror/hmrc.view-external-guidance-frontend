...
class PageBuilderSpec extends BaseSpec with ProcessJson with StanzaHelper {
  ...
  "PageBuilder" must {
        ...
        val process = Process(metaSection, flow, Vector[Phrase]( phrase1, phrase2 ), Vector[Link]())
         ...
         PageBuilder.pages( process ) match {
           case Right(pages) => {
             ...
             assert( newPage.stanzas.size == 4 )

             newPage.stanzas(1) mustBe Instruction( instructionStanza1, phrase1, None )
             newPage.stanzas(2) mustBe Instruction( instructionStanza2, phrase2, None )
           }
           case Left(err) => fail( s"Flow error $err" )
         }
    }
    ...
    "Return grouped Instruction stanzas when text contents start with similar text" in {
      ...
      PageBuilder.pages( process ) match {
        case Right(pages) => {
          ...
          newPage.stanzas(1) mustBe expectedInstructionGroup
        }
        case Left(err) => fail( s"Flow error $err" )
      }
    }
    ...
    "Correctly group instructions stanzas in a complex page" in {
      ...
      val instructionStanza1: InstructionStanza = InstructionStanza( 1, Seq( "3" ), None, false )
      val instructionStanza2: InstructionStanza = InstructionStanza( 2, Seq( "4" ), None, false )
      ...
      PageBuilder.pages( process ) match {
        case Right(pages) => {
          ...
          assert( newPage.stanzas.size == 10 )
          ...
          newPage.stanzas(2) mustBe expectedInstructionGroup1
          ...
          newPage.stanzas( eight ) mustBe expectedInstructionGroup2
        }
        case Left(err) => fail( s"Flow error $err" )
      }
