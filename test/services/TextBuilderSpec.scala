...
class TextBuilderSpec extends BaseSpec {
  ...
  "TextBuilder placeholder parsing" must {
    ...
    "Convert button link in tab placeholders within phrase to Link as button TextItems" in new Test {
      ...
      txt.english shouldBe Seq(Words("Sentence with a "), Link("dummy-path", "BLAH", true, true), Words(" label reference"))
      txt.welsh shouldBe Seq(Words("Sentence with a "), Link("dummy-path", "BLAH", true, true), Words(" label reference"))
